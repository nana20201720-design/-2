...
export const ACHIEVEMENTS_LIST: Achievement[] = [
  ...
  },
  {
    ...
    titleAr: 'أفضل قناص',
    titleEn: 'Best Sniper',
    descriptionAr: 'حقق 15 قتلة بضربات رأس قاتلة بدقة متناهية.',
    badgeIcon: '🏅',
    iconName: 'Target',
    category: 'skill',
    color: 'from-amber-600 via-orange-500 to-yellow-500',
    rewardXP: 450,
    condition: (stats) => stats.totalHeadshots >= 15,
    progressText: (stats) => `${Math.min(stats.totalHeadshots, 15)} / 15 قتلة رأس`,
    progressPercent: (stats) => Math.min(100, Math.floor((stats.totalHeadshots / 15) * 100)),
  },
  {
    id: 'last_survival',
    titleAr: 'البقاء الأخير',
    titleEn: 'Sole Survivor',
    descriptionAr: 'حقق 10 انتصارات حاسمة وكن البطل الأخير الصامد في الساحة.',
    badgeIcon: '🎖️',
    iconName: 'Shield',
    category: 'survival',
    color: 'from-teal-500 via-emerald-600 to-green-500',
    rewardXP: 600,
    condition: (stats) => stats.totalWins >= 10,
    progressText: (stats) => `${Math.min(stats.totalWins, 10)} / 10 انتصارات ساحقة`,
    progressPercent: (stats) => Math.min(100, Math.floor((stats.totalWins / 10) * 100)),
  },
];
